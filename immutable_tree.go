...
// TraverseStateChanges iterate the range of versions, compare each version to it's predecessor to extract the state changes of it.
// endVersion is exclusive.
func (t *ImmutableTree) TraverseSeparateStateChanges(startVersion, endVersion int64, fn func(pair *KVPair) error) error {
	return t.ndb.traverseSeperateStateChanges(startVersion, endVersion, fn)
}
